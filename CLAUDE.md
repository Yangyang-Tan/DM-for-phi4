...
- **Empirical sigma rule:** pick σ so that `std(t=1) ≈ 2·D_max_norm`, where `D_max_norm` is the max pairwise Euclidean distance of training cfgs in the normalised [-1,1] space.
...
- **Device-selection policy:**
  ...
  - **One job per GPU.** never co-locate two compute jobs on the same device
