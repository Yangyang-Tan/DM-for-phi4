"""
Track MALA acceptance rate vs training epoch.

Uses fixed (c, t_mh) to measure score quality at each saved checkpoint.

Usage:
    python acceptance_vs_epoch.py --device cuda:0 --L 128 --k 0.28
    python acceptance_vs_epoch.py --device cuda:0 --L 128 --k 0.28 --c 0.5 --t_mh 1e-4
"""
...
def main():
    ...
    parser.add_argument("--epochs", type=int, nargs='*', default=None,
                        help="Specific epochs to evaluate (default: all)")
    ...
    if args.epochs:
        ...
        # Select checkpoints at fixed epoch interval, picking the nearest available
        step = args.every
        target = all_epochs[0]
        epochs = []
        while target <= all_epochs[-1]:
            # Find the closest available epoch to the target
    ...
    # Load reference data (fixed seed for reproducibility)
    ...
    out_csv = os.path.join(SCRIPT_DIR, f"acceptance_vs_epoch_L{L}_k{args.k}.csv")
    ...
    ax.set_title(rf'$L={L},\;\kappa={args.k}$')
    ax.set_ylim(-0.05, 1.05)
    ax.grid(alpha=0.3)
    fig.tight_layout()

    out_plot = os.path.join(SCRIPT_DIR, f"acceptance_vs_epoch_L{L}_k{args.k}.pdf")
