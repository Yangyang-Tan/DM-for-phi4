"""
Calibrate t_mh by sweeping over evaluation time on an early vs late checkpoint.

Fixes h = c/L² (default c=0.5) and sweeps t_mh to find the value with
the best diagnostic power (largest acceptance gap between early and late).

Usage:
    python calibrate_t_mh.py --device cuda:0 --L 128 --k 0.28
    python calibrate_t_mh.py --device cuda:0 --L 128 --k 0.28 --c 0.5
"""
...
def sweep_t_mh(model, action_fn, t_mh_values, x_ref, mh_steps, h):
    """Run MALA at fixed step size, sweeping t_mh."""
    results = []
    device = model.device
    num_samples = x_ref.shape[0]
    h_t = torch.tensor(h, device=device, dtype=x_ref.dtype)

    for t_mh in t_mh_values:
        x = x_ref.clone()
        ...
        sem = (acc.std() / acc.shape[0] ** 0.5).item()
        results.append((acc.mean().item(), sem))
        print(f"    t_mh = {t_mh:.1e}  →  accept = {acc.mean().item():.4f} "
              f"± {sem:.4f}")
...
def main():
    ...
    parser.add_argument("--c", type=float, default=0.5,
                        help="Fixed step size coefficient h = c / L²")
    ...
    res_early = sweep_t_mh(model_early, action_fn, t_mh_values, x_ref,
                           args.mh_steps, h)
    del model_early
    ...
    res_late = sweep_t_mh(model_late, action_fn, t_mh_values, x_ref,
                          args.mh_steps, h)
    del model_late
    torch.cuda.empty_cache()
    ...
    ax1.errorbar(t_mh_values, acc_early,
                 yerr=[r[1] for r in res_early],
                 label=f'Epoch {early_ep} (early)', marker='o',
                 capsize=3, color='C3')
    ax1.errorbar(t_mh_values, acc_late,
    ...
    ax1.set_title(rf'$L={L},\;\kappa={args.k},\;c={args.c}$')
    ...
    for t in t_mh_values:
        ...
        m = t / 10**e
        if m == 1:
            bar_labels.append(rf'$10^{{{e}}}$')
        else:
            bar_labels.append(rf'${m:.0f}\!\times\!10^{{{e}}}$')
    ax2.bar(range(len(t_mh_values)), gap,
            tick_label=bar_labels, color=colors)
    ...
    fig.tight_layout()
    out_path = os.path.join(SCRIPT_DIR, f"calibrate_t_mh_L{L}_k{args.k}.pdf")
