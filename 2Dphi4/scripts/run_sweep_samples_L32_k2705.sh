#!/usr/bin/env bash
# Sweep sampling for L=32 k=0.2705 sigma ablation.
# Every 5th log-scale ckpt + last, 512 samples, SDE (em 2000) + ODE (dpm2 400).
#
# 3-GPU split (mirrors training assignment):
#   cuda:0 -> σ=15, 90
#   cuda:1 -> σ=30, 180
#   cuda:3 -> σ=60, 360
set -e
...
for SIGMA in "${SIGMAS[@]}"; do
    ...
    n="${#CKPTS[@]}"
    [[ $n -eq 0 ]] && { echo ">>> [SKIP] empty"; continue; }
    ...
    echo ">>> [$(date +%F\ %T)] sigma=${SIGMA}  total=${n}  selected=${#SELECTED[@]}" | tee -a "$SWEEP_LOG"
