#!/usr/bin/env bash
# Sigma ablation for 2Dphi4 L=32 k=0.28 l=0.022.
# Auto-resumes from last log-scale ckpt if one exists.
#
# 5-GPU distribution:
#   cuda:0 (4090)   ->  90
#   cuda:1 (4090)   ->  180
#   cuda:3 (5090)   ->  360, 60     (two, 5090 is fastest)
#   cuda:4 (2080Ti) ->  15
#   cuda:5 (2080Ti) ->  30
set -e
...
GPU=${1:?"usage: $0 <gpu_idx>   (0 | 1 | 3 | 4 | 5)"}
DEVICE="cuda:${GPU}"
...
case "${GPU}" in
    ...
    4) SIGMAS=(15)      ;;
    5) SIGMAS=(30)      ;;
    *) echo "gpu_idx must be 0, 1, 3, 4, or 5"; exit 1 ;;
esac
...
for SIGMA in "${SIGMAS[@]}"; do
    ...
    # Auto-resume: pick the latest ckpt if present
    RESUME_FLAG=""
