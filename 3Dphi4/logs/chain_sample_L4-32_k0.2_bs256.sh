#!/bin/bash
# Wait for L4-32 k=0.2 bs=256 resume training (PID $TRAIN_PID), then sample L=64 with linear+log on cuda:0.
set -u
TRAIN_PID=1890300
DEV=cuda:2
K=0.2
RUN_DIR=runs/phi4_3d_Lmulti4-8-16-32_k${K}_l0.9_ncsnpp_bs256
...
EP=20000
if [ ! -f "$RUN_DIR/models/epoch=$(printf '%05d' $EP).ckpt" ] && \
   [ ! -f "$RUN_DIR/models/epoch=$(printf '%04d' $EP).ckpt" ]; then
