"""
Generate samples from a trained CelebA diffusion model.

Usage:
    python sample_celeba.py --ep "epoch=0499" --image_size 64 --network ncsnpp
    python sample_celeba.py --ep "epoch=0999" --image_size 128 --method pc
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sample from trained CelebA diffusion model")
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument("--num_samples", type=int, default=1024)
    parser.add_argument("--num_steps", type=int, default=1000)
    parser.add_argument("--method", type=str, default="em",
                        choices=["em", "pc", "dpm1", "dpm2", "dpm3", "rk45"],
                        help="em (SDE), pc (predictor-corrector), "
                             "dpm1/dpm2/dpm3 (DPM-Solver ODE 1st/2nd/3rd order), "
                             "rk45 (adaptive scipy ODE)")
    parser.add_argument("--ep", type=str, default=None)
    parser.add_argument("--image_size", type=int, default=128, choices=[64, 128])
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--network", type=str, default="ncsnpp", choices=["unet", "ncsnpp"])
    ...
    parser.add_argument("--seed", type=int, default=None,
                        help="If set, seeds torch/cuda RNG before each sample run.")
    ...
    if args.checkpoint is None:
        ...
        if not ckpts:
            raise FileNotFoundError(f"No checkpoints in {model_dir}/models/ matching '{pattern}'")
    ...
    for run_idx in range(args.n_repeats):
        ...
        with torch.no_grad():
            if args.method == "em":
                samples = model.sample(args.num_samples, args.num_steps, schedule=args.schedule)
            elif args.method == "pc":
                samples = model.sample_pc(args.num_samples, args.num_steps, schedule=args.schedule)
            elif args.method in ("dpm1", "dpm2", "dpm3", "rk45"):
                samples = model.sample_ode(args.num_samples, args.num_steps,
    ...
    ep_label = args.ep.replace("epoch=", "") if args.ep else "latest"
    samples_out = samples_np.transpose(1, 2, 0)
    output_path = f"{output_dir}/samples_{args.method}_epoch={ep_label}.npy"
    np.save(output_path, samples_out)
    print(f"\nSaved: {output_path}, shape={samples_out.shape}")
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        ax.axis('off')
    plt.suptitle(f'CelebA {L}x{L} — {args.method.upper()} epoch={ep_label}')
    plt.tight_layout()

    fig_dir = f"{model_dir}/figures"
    os.makedirs(fig_dir, exist_ok=True)
    fig_path = f"{fig_dir}/samples_{args.method}_epoch={ep_label}.png"
