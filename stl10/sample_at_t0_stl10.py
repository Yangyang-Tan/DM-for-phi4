"""
Generate STL-10 samples at different reverse-SDE stopping times t0.

Studies how G(k) converges as t0 -> 0: IR modes first, UV modes last.

Usage:
    python sample_at_t0_stl10.py --ep "epoch=0499" --output_name unlabeled_all --device cuda:0
"""
...
def sample_snapshots(model, num_samples, num_steps, t0_list, schedule='log', eps=1e-5):
    """Run reverse SDE once and save snapshots at each t0."""
    model.eval()
    device = model.device
    ...
    with model.ema.average_parameters():
        ...
        for i in tqdm(range(num_steps), desc="Sampling"):
            ...
            while next_t0_idx < len(t0_sorted) and time_step >= t0_sorted[next_t0_idx] > next_time:
                ...
                print(f"  snapshot at t0={t0_val:.4f} (step {i})")
                next_t0_idx += 1
...
def main():
    ...
    sigma = ckpt.get("hyper_parameters", {}).get("sigma", 25.0)
