"""
Generate samples from a trained STL-10 diffusion model.

Saves samples as (Lx, Ly, N) .npy files for propagator analysis.

Usage:
    python sample_stl10.py --ep "epoch=0499" --output_name unlabeled_all --network ncsnpp
    python sample_stl10.py --ep "epoch=0999" --output_name traintest_cat --method pc
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sample from trained STL-10 diffusion model")
    ...
    parser.add_argument("--output_name", type=str, default="unlabeled_all",
                        help="Name matching model dir: stl10_{output_name}_{network}/")
    ...
    L = 64
    model_dir = f"stl10_{args.output_name}_{args.network}"
    ...
    samples_np = np.concatenate(all_samples, axis=0)  # (N_total, 64, 64)

    # Save as (Lx, Ly, N)
    output_dir = f"{model_dir}/data"
    os.makedirs(output_dir, exist_ok=True)
    ep_label = args.ep if args.ep else "latest"
    samples_out = samples_np.transpose(1, 2, 0)  # (64, 64, N)
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        ax.axis('off')
    plt.suptitle(f'STL-10 {args.output_name} — {args.method.upper()} epoch={ep_label}')
    plt.tight_layout()
