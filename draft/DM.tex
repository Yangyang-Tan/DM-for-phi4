...
Diffusion models~\cite{Ho2020DDPM,Song2021SDE} are a class of generative models that learn a data distribution $p_{\text{data}}(\phi)$ by reversing a gradual noising process. The forward process progressively corrupts a data sample $\phi(0)\sim p_{\text{data}}$ into pure noise $\phi(T)$ over a time interval $t\in[0,T]$, while the reverse process learns to denoise and thereby generate new samples from the Gaussian noise. Unlike one-shot generators such as GANs or VAEs, diffusion models decompose generation into many small denoising steps, each of which is a tractable learning problem.
...
\end{equation}
where $f(\phi,t)$ is the drift coefficient, $g(t)$ the diffusion coefficient, and $w$ a standard $n$-dimensional Wiener process. Two canonical choices have emerged.

\paragraph{Variance-preserving (VP) SDE.}
With $f(\phi,t)=-\tfrac{1}{2}\beta(t)\phi$ and $g(t)=\sqrt{\beta(t)}$, equation~\eq{eq:forward_sde} reduces to a time-inhomogeneous Ornstein--Uhlenbeck process whose marginal variance is bounded. Denoting $\bar\alpha(t)=\exp\!\bigl(-\int_{0}^{t}\beta(s)\,\mathrm{d}s\bigr)$, the transition kernel is
\begin{equation}
p_{0t}\bigl(\phi(t)\,\bigl|\,\phi(0)\bigr) \;=\; \mathcal{N}\!\Bigl(\phi(t);\,\sqrt{\bar\alpha(t)}\,\phi(0),\,(1-\bar\alpha(t))\,\mathbb{1}\Bigr).
\label{eq:vp_kernel}
\end{equation}
Provided the data are normalised to unit variance, $\mathrm{Var}[\phi(t)]$ is preserved exactly at unity for all $t$. This is the continuous-time limit of the discrete DDPM formulation of Ho~\emph{et al.}~\cite{Ho2020DDPM}.

\paragraph{Variance-exploding (VE) SDE.}
With $f(\phi,t)=0$ and a diffusion coefficient whose square integrates to a prescribed variance schedule $\sigma^{2}(t)$, one obtains
\begin{equation}
\mathrm{d}\phi \;=\; \sqrt{\frac{\mathrm{d}[\sigma^{2}(t)]}{\mathrm{d}t}}\,\mathrm{d}w,
\label{eq:ve_sde_explicit}
\end{equation}
whose transition kernel is
\begin{equation}
p_{0t}\bigl(\phi(t)\,\bigl|\,\phi(0)\bigr) \;=\; \mathcal{N}\!\bigl(\phi(t);\,\phi(0),\,\sigma^{2}(t)\,\mathbb{1}\bigr).
\label{eq:ve_sde_kernel}
\end{equation}
The signal $\phi(0)$ is preserved exactly and noise of growing variance is added on top. In this convention $\sigma_{\max}$ is typically chosen following the prescription of Song \& Ermon~\cite{SongErmon2020}, $\sigma_{\max}\gtrsim\max_{i,j}\|\phi_{i}-\phi_{j}\|_{2}$, so that $p_{\sigma_{\max}}(\phi)$ covers the entire data manifold. This formulation corresponds to the NCSN/SMLD line of work~\cite{SongErmon2019}.

Throughout this work we adopt the VE form with the geometric schedule $g(t)=\sigma^{t}$ on $t\in[0,1]$ parametrised by a single scalar hyperparameter $\sigma>1$. The corresponding variance follows by direct integration,
\begin{equation}
\sigma^{2}(t) \;=\; \int_{0}^{t}\!g(s)^{2}\,\mathrm{d}s \;=\; \frac{\sigma^{2t}-1}{2\ln\sigma},
\label{eq:ve_sigma_schedule}
\end{equation}
and the endpoint value $\sigma(1)=\sqrt{(\sigma^{2}-1)/(2\ln\sigma)}$ sets the maximum noise scale realised during training. We emphasise that the hyperparameter $\sigma$ appearing in $g(t)=\sigma^{t}$ is \emph{not} to be identified with the $\sigma_{\max}$ quantity entering the Song--Ermon prescription~\cite{SongErmon2020}: the two are related by~\eq{eq:ve_sigma_schedule} evaluated at $t=1$. Equating them leads to schedules that are drastically miscalibrated for lattice data, where typical field excursions can exceed those of image data by orders of magnitude.

\subsection{Score matching}
\label{sec:dm_score_matching}

The key observation enabling reverse-time generation is that the reverse dynamics depend on the data distribution only through the time-dependent \emph{score function}
\begin{equation}
s(\phi,t) \;=\; \nabla_{\phi}\log p_{t}(\phi).
\label{eq:score_def}
\end{equation}
A neural network $s_{\theta}(\phi,t)$ is trained to approximate this score via the denoising score-matching objective~\cite{Vincent2011}
\begin{multline}
\mathcal{L}(\theta) \;=\; \mathbb{E}_{t,\,\phi(0),\,\phi(t)\mid\phi(0)}\Bigl[\lambda(t)\,\bigl\|s_{\theta}(\phi(t),t)\\[-1pt]
-\nabla_{\phi(t)}\log p_{0t}\bigl(\phi(t)\,\bigl|\,\phi(0)\bigr)\bigr\|^{2}\Bigr],
\label{eq:dsm}
\end{multline}
where $\lambda(t)>0$ is a weighting function. Because the conditional distribution $p_{0t}(\phi(t)\mid\phi(0))$ is Gaussian with known mean and variance---see~\eq{eq:vp_kernel} and~\eq{eq:ve_sde_kernel}---the target score in~\eq{eq:dsm} admits a closed form, making the objective tractable. In practice this is often reparameterised as predicting the injected noise $\epsilon$ (the ``$\epsilon$-prediction'' of DDPM) or the clean signal $\phi(0)$.

For the VE schedule adopted in this work, the conditional score is elementary,
\begin{equation}
\nabla_{\phi(t)}\log p_{0t}\bigl(\phi(t)\,\bigl|\,\phi(0)\bigr) \;=\; -\frac{\phi(t)-\phi(0)}{\sigma^{2}(t)} \;=\; -\frac{\epsilon}{\sigma(t)},
\label{eq:ve_cond_score}
\end{equation}
with $\phi(t)=\phi(0)+\sigma(t)\,\epsilon$ and $\epsilon\sim\mathcal{N}(0,\mathbb{1})$. Inserting~\eq{eq:ve_cond_score} and choosing $\lambda(t)=\sigma^{2}(t)$ to equalise the signal-to-noise ratio across the diffusion trajectory yields the practical training objective
\begin{equation}
\mathcal{L}(\theta) \;=\; \mathbb{E}_{t,\,\phi(0),\,\epsilon}\!\left[\bigl\|\sigma(t)\,s_{\theta}(\phi(t),t)+\epsilon\bigr\|^{2}\right],
\label{eq:dsm_loss_ve}
\end{equation}
which is the loss minimised in all subsequent training runs. Because the VE forward SDE has no drift, its marginals satisfy $p_{t}\to p_{0}$ as $t\to 0$, so that a converged network satisfies
\begin{equation}
s_{\theta}(\phi,t)\;\xrightarrow[t\to 0]{}\;-\,\nabla_{\phi}S(\phi).
\label{eq:score_small_t}
\end{equation}
The small-$t$ limit of the learned score therefore recovers the physical force of the lattice action as a byproduct of training on noise-denoising pairs. The precision of this limit, and the sensitivity of the downstream reverse dynamics to its residual error, underlie the diagnostics of \Sec{sec:acceptance} and \Sec{sec:score_quality}.

\subsection{Reverse-time generation}
\label{sec:dm_reverse}

Given a trained score network, samples are generated by evolving from a tractable prior $\phi(T)\sim\pi$ back to $t=0$. Two dynamics yield the same marginal distributions $\{p_{t}\}_{t\in[0,T]}$~\cite{Anderson1982,Song2021SDE}. The reverse-time SDE reads
\begin{equation}
\mathrm{d}\phi \;=\; \bigl[f(\phi,t)-g^{2}(t)\,s_{\theta}(\phi,t)\bigr]\,\mathrm{d}t \;+\; g(t)\,\mathrm{d}\bar{w},
\label{eq:reverse_sde}
\end{equation}
while the equivalent probability-flow ODE is
\begin{equation}
\frac{\mathrm{d}\phi}{\mathrm{d}t} \;=\; f(\phi,t)-\tfrac{1}{2}\,g^{2}(t)\,s_{\theta}(\phi,t).
\label{eq:prob_flow_ode}
\end{equation}
Although~\eq{eq:reverse_sde} and~\eq{eq:prob_flow_ode} share the same marginals, they differ substantially at the level of individual trajectories. The SDE is stochastic and non-invertible, but its noise injection acts as a Langevin-like correction that damps accumulated errors from an imperfect score network. The ODE is deterministic and invertible, enabling likelihood evaluation via the continuous change-of-variables formula, latent-space interpolation, and exact inversion. The ODE also admits higher-order numerical solvers (e.g.\ DPM-Solver~\cite{LuDPMSolver2022}, the Heun sampler of EDM~\cite{Karras2022EDM}), allowing high-quality generation in $10$--$35$ function evaluations.

The choice of sampler involves a trade-off. For applications demanding high \emph{spectral fidelity}---for instance when the two-point correlation function $G(p)$ of generated configurations is used as a physically meaningful quality metric, as in the propagator study of \Sec{sec:dm_propagator}---the reverse SDE, or a hybrid stochastic sampler that periodically injects noise into an ODE trajectory, tends to produce more faithful ultraviolet behaviour. A deterministic ODE integrator will faithfully reproduce any bias present in the learned score, whereas the stochasticity of the SDE provides a relaxation mechanism towards the true marginal at each time slice.
